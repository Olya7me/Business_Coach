# 🚀 Проект: Бизнес-Ментор Дмитрий Воловиков

Проект **"Бизнес-Ментор Дмитрий Воловиков"!**  
Этот сайт предназначен для презентации услуг бизнес-ментора, направленных на помощь в развитии и передаче семейного бизнеса следующему поколению.  

---<img width="1440" alt="Снимок экрана 2025-01-03 в 15 11 36" src="https://github.com/user-attachments/assets/a1d942a8-8abe-4958-a9ef-7753a70ef2c7" />

## 🌐 Развернутый проект

Вы можете просмотреть развернутый проект на GitHub Pages по следующей ссылке:  
[Перейти на сайт](https://your-username.github.io/repository-name/)

## 📂 Содержание

1. [Описание](#описание)
2. [Функциональность](#функциональность)
3. [Технологии](#технологии)
4. [Установка и запуск](#установка-и-запуск)
5. [Особенности](#особенности)
6. [Отзыв заказчика](#отзыв заказчика)
7. [Контакты](#контакты)

---

## 📝 Описание

На сайте представлена информация о Дмитрии Воловикове — профессиональном бизнес-менторе и коуче с многолетним опытом.  
Проект направлен на:  
- Привлечение новых клиентов.  
- Демонстрацию этапов работы.  
- Поддержку семейных бизнесов.  

---

## ✨ Функциональность

- 🔹 **Обо мне:** Информация о менторе и его опыте.  
- 🔹 **Кому подойдет:** Раздел для потенциальных клиентов.  
- 🔹 **План работы:** Подробный план по развитию и передаче бизнеса.  
- 🔹 **Навигация:** Удобное меню для пользователей.  

---

## 💻 Технологии

Проект построен с использованием:  

- HTML5 🖋️  
- CSS3 🎨  
- SCSS 🚀  
- Normalize.css 📏  

---

Проект не использует пакетные менеджеры или сборщики. Его можно запускать напрямую через **Live Server**.

---

## 🚀 Запуск проекта

1. Убедитесь, что у вас установлен **Live Server** в вашей рабочей среде (например, Visual Studio Code).  
2. Откройте проект в редакторе.  
3. Нажмите правой кнопкой мыши на `index.html` и выберите **"Open with Live Server"**.  
4. Откройте браузер и перейдите по адресу `http://127.0.0.1:5500/` (по умолчанию).

---

## 📝 Особенности

- 🌟 Полностью адаптивный дизайн.
- 🎨 Использование **SCSS** для стилизации.
- 🧑‍💻 Чистый и читаемый код.

---

## 📝 Отзыв заказчика

- Отзыв можно посмотреть по ссылке: [https://drive.google.com/file/d/1t_y9DnXmlAqmIBqMvcrSRcLrP1MKhoQK/view?usp=drive_link]

---

## 🤝 Контакты

🌐 Веб-сайт: [http://volovikov-coach.ru/](http://volovikov-coach.ru/)

---

Спасибо за интерес к проекту! 😊



# 🚀 Project: Business Mentor Dmitry Volovikov

Welcome to the **"Business Mentor Dmitry Volovikov"** project!  
This website is designed to showcase mentoring services aimed at helping develop and transition family businesses to the next generation.  

---

## 📂 Table of Contents

1. [Description](#description)  
2. [Features](#features)  
3. [Technologies](#technologies)  
4. [Installation and Launch](#installation-and-launch)  
5. [Project Structure](#project-structure)  
6. [Contacts](#contacts)  

---

## 📝 Description

The website provides information about Dmitry Volovikov — a professional business mentor and coach with years of experience.  
This project aims to:  
- Attract new clients.  
- Showcase the workflow stages.  
- Support family businesses.  

---

## ✨ Features

- 🔹 **About Me:** Information about the mentor and his experience.  
- 🔹 **Who It's For:** A section for potential clients.  
- 🔹 **Work Plan:** A detailed plan for business development and succession.  
- 🔹 **Navigation:** User-friendly menu for visitors.  

---

## 💻 Technologies

This project is built using:  

- HTML5 🖋️  
- CSS3 🎨  
- SCSS 🚀  
- Normalize.css 📏  

---

The project does not use package managers or build tools. It can be launched directly via **Live Server**.

---

## 🚀 Launch Instructions

1. Ensure **Live Server** is installed in your development environment (e.g., Visual Studio Code).  
2. Open the project in your editor.  
3. Right-click on `index.html` and select **"Open with Live Server"**.  
4. Open your browser and go to `http://127.0.0.1:5500/` (default address).  

---

## 📝 Highlights

- 🌟 Fully responsive design.  
- 🎨 Styling with **SCSS**.  
- 🧑‍💻 Clean and readable code.  

---

## 🤝 Contacts

🌐 Website: [http://volovikov-coach.ru/](http://volovikov-coach.ru/)  

---

Thank you for your interest in the project! 😊

